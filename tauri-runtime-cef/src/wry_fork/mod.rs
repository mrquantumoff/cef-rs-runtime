...
use crate::client::{BrowserEvent, RuntimeClientBuilder};
use cef::rc::Rc;
...
fn apply_background_color(slot: &BrowserSlot, (r, g, b, a): (u8, u8, u8, u8)) -> std::result::Result<(), &'static str> {
...
fn collect_cookies(slot: &BrowserSlot, url: Option<&str>) -> std::result::Result<Vec<Cookie<'static>>, &'static str> {
...
impl WebviewWrapper {
  ...
  fn set_background_color(&self, rgba: (u8, u8, u8, u8)) -> std::result::Result<(), &'static str> {
  ...
  fn cookies_for_url(&self, url: &str) -> std::result::Result<Vec<Cookie<'static>>, &'static str> {
...
fn create_webview<T: UserEvent>(
    ...
    .on_event({
      ...
      move |event| match event {
        ...
        BrowserEvent::ProcessMessage {
          ...
          let is_ipc_message =
            matches!(name.as_str(), "tauri-ipc" | "__TAURI_IPC__" | "tauri:ipc" | "ipc");
          if !is_ipc_message {
            return;
          }
  ...
  #[cfg(feature = "tao-runtime")]
  {
    if let Ok(host_window) = HostWindowInfo::from_tao_window(window) {
      ...
      let window_info = WindowInfo::default().set_as_child(host_window.parent_handle, &cef_bounds);
      let url = CefString::from(url.as_str());
